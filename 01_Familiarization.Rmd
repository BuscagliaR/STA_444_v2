...
The R code in an RMD document is nicely separated from regular text using three back ticks and an instruction that it is R code that needs to be evaluated. A new R chunk can also be quickly inserted into an RMD document using the hotkey `Ctrl+Alt+I` on windows, and a green insert function is located at the top of the RMD document, or you can always manually type them in as well. The output of this document looks good as a HTML, PDF, or MS Word document. This entire book is created using RMarkdown. To see what the the RMD file looks like for any chapter, just click on the pencil icon at the top of the online notes. This is a great way to view the source code for anything programmed in this book, and is highly recommended for copy/pasting information from the book into your own work, such as the exercises at the end of each chapter.
