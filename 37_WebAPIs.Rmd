...
Dr. Sonderegger's Video Companion: [Video Lecture](https://youtu.be/BlWk25GI3HY).
...
CENSUS_API_KEY = '88cf659387c8713905286d5aa442597f4e8c11fd'
```
...
## Package `tidycensus`
The `tidycensus` package is a little easier to work with.
By default, `tidycensus` functions will look for the API Key
...
```
```{r, echo=FALSE}
# Actually read in my API key that is stored in my .Rprofile file.
source('.Rprofile')
...
For our first example, we'll grab the county population totals in Arizona broken
down by sex and ethnicity. 
...
For our next example, we'll grab the number of people with or without Insurance
in each state.
```{r}
