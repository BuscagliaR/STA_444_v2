...
In this chapter we will learn to handle factors, a key data structure in programming languages to handled strings within models and analysis in a light-weight and flexible manner.  The chapter primarily leverages the functionality within the `forcats` package of the `tidyverse` suite.
...
Dr. Sonderegger's Video Companion: [Video Lecture](https://youtu.be/ID2hx2ySAEE).
...
To demonstrate the `forcats` functions, we will consider two data sets. The first consists of a data set of a small US high school with observations for each student that record their year and gender.
...
# In the order of the counts of each level
Students %>%
...
For a second example data set, consider a September 2019 [poll](https://www.monmouth.edu/polling-institute/reports/monmouthpoll_NH_092419/) from Monmouth University of New Hampshire Democrats and Independents.
...
We might consider moving the `Other` and `Undecided` categories to below "No one". 
